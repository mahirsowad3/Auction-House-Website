...
export default function AddItem() {
  ...
  // to hndle add item button disabled
  useEffect(() => {
  ...
  const addItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // function to add basic item info
    ...
    const addImages = async (itemId: Number, imageURL: any) => {
      const payload = { "body": {
        "username": "Test1",
        "password": "Test1",
        "relatedItem": itemId,
    ...
    console.log("basicItemInfo: ", basicItemInfo);
    console.log("imageFiles: ", imageFiles);
    for (let i = 0; i < imageFiles.length; i++) {
      const image = await addImages(Number(16), imageFiles[i].name);
      ...
      <div className="container mx-auto mt-5">
        ...
        <form onSubmit={addItem}>
            <div className="grid gap-6 mb-6 md:grid-cols-1">
                ...
                <div>
                    <label htmlFor="intial_price" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Initial Price</label>
                    <input type="text" id="intial_price" value={initialPrice} onChange={(event) => {
                ...
                <div>
                    ...
                    <input type="file" id="images" ref={imageUploadRef} onChange={(event) => {
